...
from eval_metric import compute_metric_stmt, preprocess_predictions_for_pass
...
def parse_args():
    ...
    parser.add_argument('--repos_to_keep', type=str, nargs='+', default=['amazon-science_patchcore-inspection', 'deepmind_tracr', 'facebookresearch_omnivore', 'leopard-ai_betty', 'maxhumber_redframes'], help='repos to calculate pass metric')

    args = parser.parse_args()
    return args


def main_worker(rank, world_size, tokenizer, data, args, configuration):
    ...
    # --- LOAD MODEL HERE (Per Process) ---
    # Load explicitly to the correct device or CPU first to avoid VRAM spikes
    if args.model_checkpoint is not None:
        print(f"Rank {rank}: Loading checkpoint: {args.model_checkpoint}")
        # map_location ensures we don't accidentally load everything to GPU 0
        model = LlavaCodeForConditionalGeneration.load_from_checkpoint(
            args.model_checkpoint, 
            config=configuration, 
            map_location=f'cuda:{rank}' # Directly load to correct GPU
        )
    else:
    ...
    # No DDP wrapper needed for inference
    # -------------------------------------
    ...
    for entry in tqdm(data_per_rank, desc=f"Rank {rank} processing"):
        with torch.no_grad(): # Ensure no gradients are stored
            ...
            predictions = tokenizer.batch_decode(cur_pred[:, cut_at:], skip_special_tokens=True)
            cleaned_preds = [truncate_trash(p) for p in predictions]
            all_preds.append({
...
if __name__ == "__main__":
    ...
    # Setup tokenizers/config in main process (lightweight)
    ...
    assert len(code_tokenizer) <= text_config.vocab_size, 'Resize embeddings needed'
    ...
    # Pre-calculate dataset in main process (CPU only)
    ...
    # Pass configuration instead of model
    torch.multiprocessing.spawn(
        main_worker, 
        nprocs=world_size, 
        args=(world_size, code_tokenizer, data, args, configuration) 
    )
    preprocess_predictions_for_pass(args)
